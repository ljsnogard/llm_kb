...
- **「新会话」是一个草稿**：点它不会马上在 `kb_core` 上建会话——只有名字、没有
  消息的空会话不允许落盘。用户发出第一个问题时，客户端才把那条消息作为会话的
  首条内容提交给 `kb_core`，会话名由 `kb_core` 从**问题的前若干字**推导。会话行
  悬停的改名按钮可以随时改标题（留空则交回服务端重新推导）。
...
flutter pub get
flutter analyze
flutter test                        # 49 项；golden 组默认跳过
flutter run -d linux
```
